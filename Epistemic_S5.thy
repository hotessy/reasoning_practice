...
abbreviation mnot :: "\<sigma> \<Rightarrow> \<sigma>" ("\<^bold>\<not>_"[52]53) where "\<^bold>\<not> \<phi> \<equiv> (\<lambda>w. \<not> \<phi> w)"
  ...
  abbreviation mbox :: "(i \<Rightarrow> i \<Rightarrow> bool) \<Rightarrow> \<sigma> \<Rightarrow> \<sigma>" ("\<^bold>\<box>_ _") where "\<^bold>\<box>r \<phi> \<equiv> (\<lambda>w. \<forall>v. r w v \<longrightarrow> \<phi> v)"
  abbreviation mdia :: "(i\<Rightarrow>i \<Rightarrow> bool)\<Rightarrow>\<sigma> \<Rightarrow> \<sigma>" ("\<^bold>\<diamond>_ _") where "\<^bold>\<diamond>r \<phi> \<equiv> (\<lambda>w. \<exists>v. r v w \<and> \<phi> v)"
